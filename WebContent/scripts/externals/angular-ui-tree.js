...
(function () {
  ...
  angular.module('ui.tree')
    ...
    * angular-ui-tree.
    */
    .factory('$uiTreeHelper', ['$document', '$window', '$rootScope',
      function ($document, $window, $rootScope) {
        return {
          ...
          dragInfo: function(node) {
            return {
              ...
              apply: function() {
                ...
                if(this.source.node.nodeType=="test") {
                	if(this.prev() && (this.prev().node.nodeType == "archiveRoot" 
                		|| this.prev().node.nodeType == "archiveFolder" 
                			|| this.prev().node.nodeType == "archiveTest")) {
                		return;
                	} 
                	if(this.next() && (this.next().node.nodeType == "archiveFolder" || this.next().node.nodeType == "archiveTest")) {
                		return;
                	}
                }
    ...
    function(treeConfig, $window) {
      return {
        ...
        link: function(scope, element, attrs) {
          ...
          callbacks.dropped = function(event, prev, next) {
          	//on drop successful, emit dragEnd to be handled by other controllers
        	  var destParent = event.dest.nodesScope.$parent;
        	  var source = event.source.nodeScope;
        	  var sourceParent = event.source.nodeScope.$parent.$nodeScope;
        	  var sourceIndex = event.source.index;
        	  var destIndex = event.dest.index;
        	  scope.$emit('dragEnd', 
        			  destParent, source, sourceParent,
        ...
        return {
          ...
          link: function(scope, element, attrs, controllersArr) {
            ...
            var dragStart = function(e) {
              ...
              if (eventScope.$type !== 'uiTreeNode' && eventScope.$type !== 'uiTreeHandle') { // Check if it is a node or a handle
                return;
              }
            ...
            var dragMove = function(e) {
              ...
              if (dragElm) {
                ...
                if (treeScrollTop>0 && 
                    leftElmPos > $('#tree2-root')[0].offsetWidth && 
                    ((treeScrollTop> eventObj.pageY) || 
                      ($('#tree2-root').offset().top  < eventObj.pageY ))) {               
            ...
            var dragEnd = function(e) {
              ...
              if (dragElm) {
                ...
                if (scope.$$apply) {
                  dragInfo.apply();
                  scope.$treeScope.$apply(function() {
                    scope.$callbacks.dropped(dragInfo.eventArgs(elements, pos), dragInfo.prev(), dragInfo.next());
                  });
                } else {
                  bindDrag();
                }
