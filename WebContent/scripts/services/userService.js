		...
		['$http', '$rootScope', '$location', '$cookieStore', 
		 function($http, $rootScope, $location, $cookieStore) {
			...
			this.userBookIDs = function(callback) {
				...
				$http.get(
						evalu8config.host + '/settings/books', config)
						.success(
								function(response) {
			...
			this.userQuestionMetadata = function(callback) {				
				...
				$http.get(
						evalu8config.host + "/settings/questionmetadata/", config)
						.success(
								function(response) {
			...
			this.saveUserBooks = function(userBookIDs) {
				 ...
				 $http.post(evalu8config.host + '/settings/books', userBookIDs, config)
				 .success(function(response) {
					 document.getElementById("divSaveMessage").innerHTML 
			...
			this.saveUserDisciplines = function(userDisciplines) {
				 ...
				 $http.post(evalu8config.host + '/settings/disciplines', userDisciplines, config)
				 .success(function(response) {
					 document.getElementById("divSaveMessage").innerHTML 
					 = "<span style='color:green'>Settings saved successfully<span>";
				 })
			...
			this.saveUserQuestionMetadata = function(userQuestionMetadata) {
				
				 $http.post(evalu8config.host + '/settings/questionmetadata', userQuestionMetadata, config)
				 .success(function(response) {
					 document.getElementById("divSaveMessage").innerHTML 
					 = "<span style='color:green'>Settings saved successfully<span>";
				 })
