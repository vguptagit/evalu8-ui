...
.module('e8UserPref', ["checklist-model"])
...
.controller(
		...
		[
		 ...
		 function($scope, $rootScope, $location, $routeParams, $http) {
			 ...
			 $http.get(evalu8config.host + '/disciplines', config)
			 ...
			 $http.get(evalu8config.host + '/settings', config)
			 .success(function(response) {
				 $scope.userpref = response;

				 $scope.disciplines.userSelected = $scope.userpref.disciplines;
				 
				 $scope.questionMetadata.userSelected = $scope.userpref.questionMetadata;
			 ...
			 $scope.savePref = function() {								 
				 ...
				 var preference = {
						 ...
						 "questionMetadata" : $scope.questionMetadata.userSelected,
						 "disciplines" : $scope.disciplines.userSelected
				 };
				 ...
				 $http.post(evalu8config.host + '/settings', preference, config)
				 .success(function(response) {
					 document.getElementById("divSaveMessage").innerText = "Settings saved successfully";
				 })
